...
pub fn create_rc_im_list<T>() -> ConsList<Rc<RefCell<T>>> where T: Debug, T: PartialEq {
    return ConsList::<Rc<RefCell<T>>>::create();
}

pub fn create_rc_im_list_from_vec<T>(arr: &Vec<T>) -> ConsList<Rc<RefCell<T>>> where T: Debug + Clone, T: PartialEq{
...
pub fn values<T>(im_arr: &Vec::<Rc<RefCell<T>>>) -> Vec<T> where T: Clone {
...
pub fn create_rc_ro_list<T>() -> ConsList<Rc<T>> where T: Debug, T: PartialEq {
    return ConsList::<Rc<T>>::create();
}

pub fn create_rc_ro_list_from_vec<T>(arr: &Vec<T>) -> ConsList<Rc<T>> where T: Debug + Clone, T: PartialEq{
...
pub fn ro_values<T>(im_arr: &Vec::<Rc<T>>) -> Vec<T> where T: Clone {
    let mut new_arr = Vec::<T>::new();
