...
impl fmt::Display for RomanNumeral {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        let numeral_str = self
            .content
            .iter()
            .map(|roman_digit| roman_digit.as_char())
            .collect::<String>();
        write!(f, "{}", numeral_str)
    }
}
...
impl TryFrom<Vec<RomanDigit>> for RomanNumeral {
    ...
    fn try_from(digits: Vec<RomanDigit>) -> Result<Self, Self::Error> {
        ...
        if !digits.is_empty() {
            let numeral_str: String = digits
                .iter()
                .map(|roman_digit| roman_digit.as_char())
                .collect();
            let numeral = Self::from_string(&numeral_str);
            result = if !numeral.empty() {
                Ok(numeral)
            } else {
...
impl NumberToRomanNumeralConverter {
    ...
    pub fn convert(&mut self, number: u16) -> Result<RomanNumeral, MaxNumberExceededError> {
        ...
        let result = if !max_nr_exceeded {
            Ok(RomanNumeral::from_string(&self.result_str))
        } else {
    ...
    fn handle_same_appended_char_threshold(
        &mut self,
        threshold: u16,
        char_to_append: char,
    ) -> bool {
        ...
        if can_handle {
            ...
            utilities::push_multiple_times_to_vec(
                &char_to_append,
                (self.remainder / threshold) as usize,
                &mut chars_to_append,
            );
            self.result_str
                .push_str(&chars_to_append.iter().collect::<String>());
    ...
    fn handle_different_appended_chars_threshold(
        &mut self,
        threshold: u16,
        chars_to_append: &str,
    ) -> bool {
...
pub fn convert_number_to_roman_numeral_using_hash(
    number: u16,
) -> Result<RomanNumeral, MaxNumberExceededError> {
    ...
    if number > 0 && number < 5000 {
        ...
        result_str = digits
            ...
            .map(|(index, digit)| {
                ...
                ROMAN_NUMERALS_CONVERSION_MAP
                    .get(&[power_of_ten_index, *digit])
                    .unwrap_or(&"")
                    .to_string()
            })
            .collect();
    ...
    // out-of-range is treated in different ways: 0 results in empty numeral while exceeding threshold is considered an error
    let result = if !max_nr_exceeded {
        Ok(RomanNumeral::from_string(&result_str))
    } else {
        Err(MaxNumberExceededError)
    };
    result
...
impl RomanNumeralToNumberConverter {
    ...
    pub fn create() -> Self {
        Self {
            resulting_digit_values: [0; Self::MAX_DIGITS_COUNT],
            conversion_map: Self::build_numbers_conversion_map(),
        }
    }
    ...
    pub fn convert(&mut self, numeral: &RomanNumeral) -> u16 {
        ...
        if !numeral.empty() {
            ...
            result = (0..Self::MAX_DIGITS_COUNT)
                .map(|index| {
                    u16::pow(10, (Self::MAX_DIGITS_COUNT - 1 - index) as u32)
                        * self.resulting_digit_values[index]
                })
                .sum()
        }
    ...
    fn update_digit(&mut self, digit: &DecimalDigit) {
        match digit {
            DecimalDigit::Thousands(thousands_value) => {
                self.resulting_digit_values[0] = *thousands_value
            }
            DecimalDigit::Hundreds(hundreds_value) => {
                self.resulting_digit_values[1] = *hundreds_value
            }
    ...
    fn build_numbers_conversion_map() -> HashMap<(DecimalDigit, RomanDigit), DecimalDigit> {
        let result = HashMap::from([
            (
                (DecimalDigit::Thousands(0), RomanDigit::M),
                DecimalDigit::Thousands(1),
            ),
            (
                (DecimalDigit::Thousands(1), RomanDigit::M),
                DecimalDigit::Thousands(2),
            ),
            (
                (DecimalDigit::Thousands(2), RomanDigit::M),
                DecimalDigit::Thousands(3),
            ),
            (
                (DecimalDigit::Thousands(3), RomanDigit::M),
                DecimalDigit::Thousands(4),
            ),
            (
                (DecimalDigit::Thousands(0), RomanDigit::D),
                DecimalDigit::Hundreds(5),
            ),
            (
                (DecimalDigit::Thousands(1), RomanDigit::D),
                DecimalDigit::Hundreds(5),
            ),
            (
                (DecimalDigit::Thousands(2), RomanDigit::D),
                DecimalDigit::Hundreds(5),
            ),
            (
                (DecimalDigit::Thousands(3), RomanDigit::D),
                DecimalDigit::Hundreds(5),
            ),
            (
                (DecimalDigit::Thousands(4), RomanDigit::D),
                DecimalDigit::Hundreds(5),
            ),
            (
                (DecimalDigit::Thousands(0), RomanDigit::C),
                DecimalDigit::Hundreds(1),
            ),
            (
                (DecimalDigit::Thousands(1), RomanDigit::C),
                DecimalDigit::Hundreds(1),
            ),
            (
                (DecimalDigit::Thousands(2), RomanDigit::C),
                DecimalDigit::Hundreds(1),
            ),
            (
                (DecimalDigit::Thousands(3), RomanDigit::C),
                DecimalDigit::Hundreds(1),
            ),
            (
                (DecimalDigit::Thousands(4), RomanDigit::C),
                DecimalDigit::Hundreds(1),
            ),
            (
                (DecimalDigit::Thousands(0), RomanDigit::L),
                DecimalDigit::Tens(5),
            ),
            (
                (DecimalDigit::Thousands(1), RomanDigit::L),
                DecimalDigit::Tens(5),
            ),
            (
                (DecimalDigit::Thousands(2), RomanDigit::L),
                DecimalDigit::Tens(5),
            ),
            (
                (DecimalDigit::Thousands(3), RomanDigit::L),
                DecimalDigit::Tens(5),
            ),
            (
                (DecimalDigit::Thousands(4), RomanDigit::L),
                DecimalDigit::Tens(5),
            ),
            (
                (DecimalDigit::Thousands(0), RomanDigit::X),
                DecimalDigit::Tens(1),
            ),
            (
                (DecimalDigit::Thousands(1), RomanDigit::X),
                DecimalDigit::Tens(1),
            ),
            (
                (DecimalDigit::Thousands(2), RomanDigit::X),
                DecimalDigit::Tens(1),
            ),
            (
                (DecimalDigit::Thousands(3), RomanDigit::X),
                DecimalDigit::Tens(1),
            ),
            (
                (DecimalDigit::Thousands(4), RomanDigit::X),
                DecimalDigit::Tens(1),
            ),
            (
                (DecimalDigit::Thousands(0), RomanDigit::V),
                DecimalDigit::Units(5),
            ),
            (
                (DecimalDigit::Thousands(1), RomanDigit::V),
                DecimalDigit::Units(5),
            ),
            (
                (DecimalDigit::Thousands(2), RomanDigit::V),
                DecimalDigit::Units(5),
            ),
            (
                (DecimalDigit::Thousands(3), RomanDigit::V),
                DecimalDigit::Units(5),
            ),
            (
                (DecimalDigit::Thousands(4), RomanDigit::V),
                DecimalDigit::Units(5),
            ),
            (
                (DecimalDigit::Thousands(0), RomanDigit::I),
                DecimalDigit::Units(1),
            ),
            (
                (DecimalDigit::Thousands(1), RomanDigit::I),
                DecimalDigit::Units(1),
            ),
            (
                (DecimalDigit::Thousands(2), RomanDigit::I),
                DecimalDigit::Units(1),
            ),
            (
                (DecimalDigit::Thousands(3), RomanDigit::I),
                DecimalDigit::Units(1),
            ),
            (
                (DecimalDigit::Thousands(4), RomanDigit::I),
                DecimalDigit::Units(1),
            ),
            (
                (DecimalDigit::Hundreds(1), RomanDigit::C),
                DecimalDigit::Hundreds(2),
            ),
            (
                (DecimalDigit::Hundreds(2), RomanDigit::C),
                DecimalDigit::Hundreds(3),
            ),
            (
                (DecimalDigit::Hundreds(1), RomanDigit::D),
                DecimalDigit::Hundreds(4),
            ),
            (
                (DecimalDigit::Hundreds(5), RomanDigit::C),
                DecimalDigit::Hundreds(6),
            ),
            (
                (DecimalDigit::Hundreds(6), RomanDigit::C),
                DecimalDigit::Hundreds(7),
            ),
            (
                (DecimalDigit::Hundreds(7), RomanDigit::C),
                DecimalDigit::Hundreds(8),
            ),
            (
                (DecimalDigit::Hundreds(1), RomanDigit::M),
                DecimalDigit::Hundreds(9),
            ),
            (
                (DecimalDigit::Hundreds(1), RomanDigit::L),
                DecimalDigit::Tens(5),
            ),
            (
                (DecimalDigit::Hundreds(2), RomanDigit::L),
                DecimalDigit::Tens(5),
            ),
            (
                (DecimalDigit::Hundreds(3), RomanDigit::L),
                DecimalDigit::Tens(5),
            ),
            (
                (DecimalDigit::Hundreds(4), RomanDigit::L),
                DecimalDigit::Tens(5),
            ),
            (
                (DecimalDigit::Hundreds(5), RomanDigit::L),
                DecimalDigit::Tens(5),
            ),
            (
                (DecimalDigit::Hundreds(6), RomanDigit::L),
                DecimalDigit::Tens(5),
            ),
            (
                (DecimalDigit::Hundreds(7), RomanDigit::L),
                DecimalDigit::Tens(5),
            ),
            (
                (DecimalDigit::Hundreds(8), RomanDigit::L),
                DecimalDigit::Tens(5),
            ),
            (
                (DecimalDigit::Hundreds(9), RomanDigit::L),
                DecimalDigit::Tens(5),
            ),
            (
                (DecimalDigit::Hundreds(1), RomanDigit::X),
                DecimalDigit::Tens(1),
            ),
            (
                (DecimalDigit::Hundreds(2), RomanDigit::X),
                DecimalDigit::Tens(1),
            ),
            (
                (DecimalDigit::Hundreds(3), RomanDigit::X),
                DecimalDigit::Tens(1),
            ),
            (
                (DecimalDigit::Hundreds(4), RomanDigit::X),
                DecimalDigit::Tens(1),
            ),
            (
                (DecimalDigit::Hundreds(5), RomanDigit::X),
                DecimalDigit::Tens(1),
            ),
            (
                (DecimalDigit::Hundreds(6), RomanDigit::X),
                DecimalDigit::Tens(1),
            ),
            (
                (DecimalDigit::Hundreds(7), RomanDigit::X),
                DecimalDigit::Tens(1),
            ),
            (
                (DecimalDigit::Hundreds(8), RomanDigit::X),
                DecimalDigit::Tens(1),
            ),
            (
                (DecimalDigit::Hundreds(9), RomanDigit::X),
                DecimalDigit::Tens(1),
            ),
            (
                (DecimalDigit::Hundreds(1), RomanDigit::V),
                DecimalDigit::Units(5),
            ),
            (
                (DecimalDigit::Hundreds(2), RomanDigit::V),
                DecimalDigit::Units(5),
            ),
            (
                (DecimalDigit::Hundreds(3), RomanDigit::V),
                DecimalDigit::Units(5),
            ),
            (
                (DecimalDigit::Hundreds(4), RomanDigit::V),
                DecimalDigit::Units(5),
            ),
            (
                (DecimalDigit::Hundreds(5), RomanDigit::V),
                DecimalDigit::Units(5),
            ),
            (
                (DecimalDigit::Hundreds(6), RomanDigit::V),
                DecimalDigit::Units(5),
            ),
            (
                (DecimalDigit::Hundreds(7), RomanDigit::V),
                DecimalDigit::Units(5),
            ),
            (
                (DecimalDigit::Hundreds(8), RomanDigit::V),
                DecimalDigit::Units(5),
            ),
            (
                (DecimalDigit::Hundreds(9), RomanDigit::V),
                DecimalDigit::Units(5),
            ),
            (
                (DecimalDigit::Hundreds(1), RomanDigit::I),
                DecimalDigit::Units(1),
            ),
            (
                (DecimalDigit::Hundreds(2), RomanDigit::I),
                DecimalDigit::Units(1),
            ),
            (
                (DecimalDigit::Hundreds(3), RomanDigit::I),
                DecimalDigit::Units(1),
            ),
            (
                (DecimalDigit::Hundreds(4), RomanDigit::I),
                DecimalDigit::Units(1),
            ),
            (
                (DecimalDigit::Hundreds(5), RomanDigit::I),
                DecimalDigit::Units(1),
            ),
            (
                (DecimalDigit::Hundreds(6), RomanDigit::I),
                DecimalDigit::Units(1),
            ),
            (
                (DecimalDigit::Hundreds(7), RomanDigit::I),
                DecimalDigit::Units(1),
            ),
            (
                (DecimalDigit::Hundreds(8), RomanDigit::I),
                DecimalDigit::Units(1),
            ),
            (
                (DecimalDigit::Hundreds(9), RomanDigit::I),
                DecimalDigit::Units(1),
            ),
            (
                (DecimalDigit::Tens(1), RomanDigit::X),
                DecimalDigit::Tens(2),
            ),
            (
                (DecimalDigit::Tens(2), RomanDigit::X),
                DecimalDigit::Tens(3),
            ),
            (
                (DecimalDigit::Tens(1), RomanDigit::L),
                DecimalDigit::Tens(4),
            ),
            (
                (DecimalDigit::Tens(5), RomanDigit::X),
                DecimalDigit::Tens(6),
            ),
            (
                (DecimalDigit::Tens(6), RomanDigit::X),
                DecimalDigit::Tens(7),
            ),
            (
                (DecimalDigit::Tens(7), RomanDigit::X),
                DecimalDigit::Tens(8),
            ),
            (
                (DecimalDigit::Tens(1), RomanDigit::C),
                DecimalDigit::Tens(9),
            ),
            (
                (DecimalDigit::Tens(1), RomanDigit::V),
                DecimalDigit::Units(5),
            ),
            (
                (DecimalDigit::Tens(2), RomanDigit::V),
                DecimalDigit::Units(5),
            ),
            (
                (DecimalDigit::Tens(3), RomanDigit::V),
                DecimalDigit::Units(5),
            ),
            (
                (DecimalDigit::Tens(4), RomanDigit::V),
                DecimalDigit::Units(5),
            ),
            (
                (DecimalDigit::Tens(5), RomanDigit::V),
                DecimalDigit::Units(5),
            ),
            (
                (DecimalDigit::Tens(6), RomanDigit::V),
                DecimalDigit::Units(5),
            ),
            (
                (DecimalDigit::Tens(7), RomanDigit::V),
                DecimalDigit::Units(5),
            ),
            (
                (DecimalDigit::Tens(8), RomanDigit::V),
                DecimalDigit::Units(5),
            ),
            (
                (DecimalDigit::Tens(9), RomanDigit::V),
                DecimalDigit::Units(5),
            ),
            (
                (DecimalDigit::Tens(1), RomanDigit::I),
                DecimalDigit::Units(1),
            ),
            (
                (DecimalDigit::Tens(2), RomanDigit::I),
                DecimalDigit::Units(1),
            ),
            (
                (DecimalDigit::Tens(3), RomanDigit::I),
                DecimalDigit::Units(1),
            ),
            (
                (DecimalDigit::Tens(4), RomanDigit::I),
                DecimalDigit::Units(1),
            ),
            (
                (DecimalDigit::Tens(5), RomanDigit::I),
                DecimalDigit::Units(1),
            ),
            (
                (DecimalDigit::Tens(6), RomanDigit::I),
                DecimalDigit::Units(1),
            ),
            (
                (DecimalDigit::Tens(7), RomanDigit::I),
                DecimalDigit::Units(1),
            ),
            (
                (DecimalDigit::Tens(8), RomanDigit::I),
                DecimalDigit::Units(1),
            ),
            (
                (DecimalDigit::Tens(9), RomanDigit::I),
                DecimalDigit::Units(1),
            ),
            (
                (DecimalDigit::Units(1), RomanDigit::I),
                DecimalDigit::Units(2),
            ),
            (
                (DecimalDigit::Units(2), RomanDigit::I),
                DecimalDigit::Units(3),
            ),
            (
                (DecimalDigit::Units(1), RomanDigit::V),
                DecimalDigit::Units(4),
            ),
            (
                (DecimalDigit::Units(5), RomanDigit::I),
                DecimalDigit::Units(6),
            ),
            (
                (DecimalDigit::Units(6), RomanDigit::I),
                DecimalDigit::Units(7),
            ),
            (
                (DecimalDigit::Units(7), RomanDigit::I),
                DecimalDigit::Units(8),
            ),
            (
                (DecimalDigit::Units(1), RomanDigit::X),
                DecimalDigit::Units(9),
            ),
        ]);
