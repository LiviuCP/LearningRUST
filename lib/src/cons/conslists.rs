...
use std::{fmt::Debug, rc::Rc, result::Result};
...
impl<T: Clone + PartialEq + std::fmt::Debug> ConsList<T> {
    ...
    pub fn push_front(&mut self, val: &T) {
        self.first = Some(Rc::new(ConsItem {
            value: val.clone(),
    ...
    pub fn insert(&mut self, val: &T, index: usize) -> Result<usize, InvalidIndex> {
        ...
        if index <= self.count {
            if self.count > 0 {
                ...
                let resulting_first_item =
                    self.stack_items(index, &mut items_preceding_insert_position);
    ...
    pub fn remove(&mut self, index: usize) -> Result<T, InvalidIndex> {
        ...
        if index < self.count {
            ...
            let resulting_first_item =
                self.stack_items(index, &mut items_preceding_remove_position);
    ...
    pub fn merge(&mut self, list: &mut ConsList<T>) {
        ...
        if let Some(mut first_item) = self.first.clone() {
            ...
            loop {
                list.first = Some(Rc::new(ConsItem {
                    value: first_item.value.clone(),
                    next: list.first.clone(),
                }));
                list.count += 1;
    ...
    fn stack_items(
        &self,
        items_to_stack_count: usize,
        stack: &mut ConsList<T>,
    ) -> Option<Rc<ConsItem<T>>> {
        ...
        if let Some(mut first_item) = self.first.clone() {
            ...
            while current_index < items_to_stack_count {
                stack.first = Some(Rc::new(ConsItem {
                    value: first_item.value.clone(),
                    next: stack.first.clone(),
                }));
                stack.count += 1;
