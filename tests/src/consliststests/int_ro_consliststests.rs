...
use learn_rust_lib::ro_conslists::{RoConsList, InvalidIndex};

#[test]
pub fn test_create() {
    let list = RoConsList::<i32>::create();
    assert!(list.content() == Vec::new() && list.size() == 0 && list.empty());
}

#[test]
pub fn test_create_from_vec() {
    let mut list = RoConsList::create_from_vec(&vec![-4, 8, -3, -3, 5, 0, 2, 1]);
    assert!(list.content() == vec![-4, 8, -3, -3, 5, 0, 2, 1] && list.size() == 8 && !list.empty());

    list = RoConsList::create_from_vec(&vec![2, 5]);
    assert!(list.content() == vec![2, 5] && list.size() == 2 && !list.empty());

    list = RoConsList::create_from_vec(&vec![-3]);
    assert!(list.content() == vec![-3] && list.size() == 1 && !list.empty());

    list = RoConsList::create_from_vec(&Vec::new());
    assert!(list.content() == Vec::new() && list.size() == 0 && list.empty());
}
...
pub fn test_iter_with_lambda() {
    let list = RoConsList::create_from_vec(&vec![-4, 8, -3, -3, 5, 0, 2, 1]);
...
pub fn test_push_front() {
    let mut list = RoConsList::create_from_vec(&vec![8, -3, -3, 5, 0, 2, 1]);
    let mut value = -4;
    list.push_front(&value);

    assert!(list.content() == vec![-4, 8, -3, -3, 5, 0, 2, 1] && list.size() == 8);

    list = RoConsList::create_from_vec(&vec![5]);
    value = 2;
    list.push_front(&value);
...
pub fn test_insert() {
    ...
    list = RoConsList::create_from_vec(&vec![8, -3, -3, 5, 0, 2, 1, -4]);
    ...
    list = RoConsList::create_from_vec(&vec![2, 4, -3]);
    value = 8;
    ...
    list = RoConsList::create_from_vec(&vec![9, 5]);
    value = -7;
    result = list.insert(&value, 0);
    ...
    list = RoConsList::create_from_vec(&vec![-4]);
    value = 5;
    result = list.insert(&value, 0);
...
#[test]
pub fn test_merge() {
    let mut first_list = RoConsList::create();
    ...
    first_list = RoConsList::create_from_vec(&vec![-3]);
    ...
    second_list = RoConsList::create_from_vec(&vec![-3]);
    first_list.merge(&mut second_list);

    assert!(first_list.content() == vec![-3] && first_list.size() == 1);
    assert!(second_list.empty());

    first_list = RoConsList::create_from_vec(&vec![5, -4]);
    second_list = RoConsList::create();
    first_list.merge(&mut second_list);
    ...
    first_list = RoConsList::create();
    second_list = RoConsList::create_from_vec(&vec![5, -4]);
    first_list.merge(&mut second_list);

    assert!(first_list.content() == vec![5, -4] && first_list.size() == 2);
    assert!(second_list.empty());

    first_list = RoConsList::create_from_vec(&vec![-8, 1]);
    second_list = RoConsList::create_from_vec(&vec![-4]);
    first_list.merge(&mut second_list);

    assert!(first_list.content() == vec![-8, 1, -4] && first_list.size() == 3);
    assert!(second_list.empty());

    first_list = RoConsList::create_from_vec(&vec![-4]);
    second_list = RoConsList::create_from_vec(&vec![-8, 1]);
    first_list.merge(&mut second_list);

    assert!(first_list.content() == vec![-4, -8, 1] && first_list.size() == 3);
    assert!(second_list.empty());

    first_list = RoConsList::create_from_vec(&vec![2, -5, 4, 3, 4]);
    second_list = RoConsList::create_from_vec(&vec![9, 1, 1, 8]);
    first_list.merge(&mut second_list);

    assert!(first_list.content() == vec![2, -5, 4, 3, 4, 9, 1, 1, 8] && first_list.size() == 9);
    assert!(second_list.empty());

    first_list = RoConsList::create_from_vec(&vec![9, 1, 1, 8]);
    second_list = RoConsList::create_from_vec(&vec![2, -5, 4, 3, 4]);
    first_list.merge(&mut second_list);
...
#[test]
pub fn test_split() {
    let mut first_list = RoConsList::create_from_vec(&vec![2, -5, 4, 3, 4, 9, 1, 1, 8]);
    let mut second_list = RoConsList::create();
    ...
    first_list = RoConsList::create_from_vec(&vec![2, -5, 4, 3, 4, 9, 1, 1, 8]);
    second_list = RoConsList::create();
    ...
    first_list = RoConsList::create_from_vec(&vec![9, -7, 8]);
    second_list = RoConsList::create();
    ...
    first_list = RoConsList::create_from_vec(&vec![6, -9]);
    second_list = RoConsList::create();
    result = first_list.split(&mut second_list, 0);
    ...
    first_list = RoConsList::create_from_vec(&vec![5]);
    second_list = RoConsList::create();
    result = first_list.split(&mut second_list, 0);
    ...
    first_list = RoConsList::create_from_vec(&vec![2, -5, 4, 3, 4, 9, 1, 1, 8]);
    second_list = RoConsList::create_from_vec(&vec![4, 0, -2, -4]);
    result = first_list.split(&mut second_list, 3);
...
#[test]
pub fn test_head() {
    let mut list = RoConsList::create_from_vec(&vec![2, 5, -3, 4]);
    assert_ne!(list.head(), None);

    let mut read_head = list.head();
    assert_eq!(*read_head.unwrap(), 2);

    list = RoConsList::create_from_vec(&vec![-8]);
    assert_ne!(list.head(), None);
...
#[test]
pub fn test_at() {
    let mut list = RoConsList::create_from_vec(&vec![9, -3, -3, 5, 0, 2, 1, -4]);
